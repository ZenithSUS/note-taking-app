        ...
        <div className="flex flex-col gap-4">
            <form className="flex flex-col bg-gray-900 p-4 mt-4 rounded-2xl gap-4 w-75 max-w-full sm:w-90 sm:max-w-md py-2 sm:py-6">
                    ...
                    {errors?.username && <p className="text-red-500 block">{errors.username}</p> ||
                        errors?.usernameValid && Object.values(errors.usernameValid).map(
                            (error: string, index: number) => (
                                <span key={index} className="text-red-500 block"> - {error}</span>
                    ))}
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="email">Email</label>
                    <input type="text" id="email" className="py-2 rounded-2xl bg-white" value={email} onChange={(event) => setEmail(event.target.value)} onInput={handleInput} />
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <input type="password" id="confirmpassword" className="py-2 rounded-2xl bg-white" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} onInput={handleInput} />
                ...
                {networkError?.message && <p className="text-red-500 text-center">{networkError.message}</p>}
            </form>
